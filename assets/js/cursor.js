...
class PointerParticles extends HTMLElement {
  ...
  constructor() {
    ...
    this.hue = 0;
    this.maxParticles = 1000;  // Limite le nombre de particules
    this.lastPointerMove = 0;  // Utilisé pour le throttling
  }
  ...
  createParticles(event, { count, speed, spread }) {
    ...
    for (let i = 0; i < count; i++) {
      if (this.particles.length >= this.maxParticles) {
        this.particles.shift();  // Supprime les anciennes particules si le maximum est atteint
      }
  ...
  setupEvents() {
    ...
    target.addEventListener("pointermove", (event) => {
      ...
      if (now - this.lastPointerMove > 16) { // Throttling : ne créer des particules qu'une fois toutes les 16ms (60fps)
        this.createParticles(event, {
          count: 20,
